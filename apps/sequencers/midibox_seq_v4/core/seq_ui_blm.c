...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  if( high_prio ) {
    ...
    // <--------------------------------------><-------------------------------------->
    SEQ_LCD_CursorSet(0, 0);
    SEQ_LCD_PrintString("This is the BLM Page by data:fux");
    SEQ_LCD_CursorSet(0, 1);

    switch( blm_mode ) {
    case BLM_MODE_GRID:
      SEQ_LCD_PrintString("Grid Mode                               ");
      break;
    case BLM_MODE_TRACKS:
      SEQ_LCD_PrintString("Track Mode                              ");
      break;
